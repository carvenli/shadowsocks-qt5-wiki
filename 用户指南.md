...
### Shadowsocks-Nodejs ###
#### Windows ####
Windows用户安装Shadowsocks-Nodejs作为后端，请通过在命令提示符`cmd`，执行`npm install -g shadowsocks`安装或升级。这样`ss-qt5`能自动找到Shadowsocks-Nodejs的后端文件`sslocal.cmd`。

`ss-qt5`会自动从`sslocal.cmd`的目录下去找真实的`sslocal`文件并调用`node`启动`sslocal`，所以请不要直接指定到`npm\node_modules\shadowsocks\bin\sslocal`。
